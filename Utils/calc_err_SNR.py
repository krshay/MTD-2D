...
# def calc_err_SNR_knownpsftsf(L, ne, N, SNRs, sd):
#     """ Calculate estimation error in estimating a specific target image, multiple SNRs. For the case of known PSF and TSF.
...
#     """
#     np.random.seed(sd)
#     errs = np.zeros((len(SNRs), 3))
#     costs = np.zeros((len(SNRs), 3))
...
# def calc_err_SNR_Algorithm1(L, ne, N, SNRs, sd):
#     """ Calculate estimation error in estimating a specific target image, multiple SNRs. For the case of Algorithm1.
...
def calc_err_SNR_bothcases(L, ne, N, SNRs, sd):
    ...
    for (idx, SNR) in enumerate(SNRs):
        ...
        print(f'iteration #{idx} in seed #{sd} is finished.')
