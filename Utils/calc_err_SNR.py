...
def calc_err_SNR_bothcases(L, ne, N, SNRs, sd):
    ...
    for (idx, SNR) in enumerate(SNRs):
            ...
            for j1 in range(L):
                for i2 in range(L):
                    for j2 in range(L):
                        M3_y[i1, j1, i2, j2] = M3_2d(yy, (i1, j1), (i2, j2))
        del y
        np.save(f'M1_y_SNR_{idx}_seed_{sd}.npy', M1_y)
        np.save(f'M2_y_SNR_{idx}_seed_{sd}.npy', M2_y)
        np.save(f'M3_y_SNR_{idx}_seed_{sd}.npy', M3_y)
        ...
        print(f'finished SNR #{idx} in seed #{sd}')
