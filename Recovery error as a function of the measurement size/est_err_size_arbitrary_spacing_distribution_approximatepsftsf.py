...
from calc_err_size import calc_err_size_approximatepsftsf
...
if __name__ == '__main__':
    ...
    S = pool.starmap(calc_err_size_approximatepsftsf, [[L, ne, N, sizes, i+100] for i in range(Niters)])
    pool.close()
    pool.join() 
    ...
    np.save('errs_approx_100_150.npy', errs)


    plt.figure()
    plt.loglog(sizes**2, errs_mean)  
    plt.loglog(sizes**2, errs_mean[0]*(sizes**2/sizes[0]**2)**(-1/2), '--')
